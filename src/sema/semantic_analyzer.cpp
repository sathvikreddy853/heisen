...
SemanticType* SemanticAnalyzer::cloneType(SemanticType* type) {
    ...
    switch (type->getKind()) {
        ...
        case SemanticType::Kind::ERROR:
            return new PrimitiveType(type->getKind());
            
        case SemanticType::Kind::ARRAY: {
            auto* arrType = static_cast<ArrayType*>(type);
            return new ArrayType(cloneType(arrType->getElementType()), 
                               arrType->getDimension());
        }
...
bool SemanticAnalyzer::isQubitType(SemanticType* type) {
    ...
    if (type->getKind() == SemanticType::Kind::QUBIT) {
        return true;
    }
...
void SemanticAnalyzer::visitForStmt(ForStmt* stmt) {
    ...
    if (stmt->getInit()) {
        visitDeclarationStmt(stmt->getInit());
    }
...
void SemanticAnalyzer::visitBreakStmt(BreakStmt* stmt) {
    if (!inLoop) {
        reportError(stmt->loc, "Break statement outside loop");
    }
}

void SemanticAnalyzer::visitContinueStmt(ContinueStmt* stmt) {
    if (!inLoop) {
        reportError(stmt->loc, "Continue statement outside loop");
    }
}
...
void SemanticAnalyzer::visitMeasureStmt(MeasureStmt* stmt) {
    ...
    if (srcType->getKind() == SemanticType::Kind::ARRAY && 
        tgtType->getKind() == SemanticType::Kind::ARRAY) {
        auto* srcArr = static_cast<ArrayType*>(srcType);
        auto* tgtArr = static_cast<ArrayType*>(tgtType);
        
        if (srcArr->getDimension() != tgtArr->getDimension() && 
            srcArr->getDimension() != -1 && tgtArr->getDimension() != -1) {
...
SemanticType* SemanticAnalyzer::visitLambdaExpr(LambdaExpr* expr) {
    ...
    // Analyze body
    if (expr->getBody()) {
        visitCompoundStmt(expr->getBody());
    }
...
SemanticType* SemanticAnalyzer::inferBinaryOpType(
    const std::string& op,
    SemanticType* left,
    SemanticType* right,
    Location loc)
{
    ...
    if (op == "+" || op == "-" || op == "*" || op == "/" || op == "%" || op == "**") {
        ...
        bool leftNumeric = (leftKind == SemanticType::Kind::INT || 
                           leftKind == SemanticType::Kind::FLOAT);
    ...
    if (op == "==" || op == "!=" || op == "<" || op == ">" || op == "<=" || op == ">=") {
        if (!isTypeCompatible(left, right)) {
            reportError(loc, "Cannot compare incompatible types '" + 
...
SemanticType* SemanticAnalyzer::inferUnaryOpType(
    const std::string& op,
