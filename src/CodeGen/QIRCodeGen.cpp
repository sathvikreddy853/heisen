...
void QIRCodeGen::declareQubitManagement() {
    ...
    runtime.qubitAllocate = llvm::Function::Create(
        llvm::FunctionType::get(qubitPtrType, {}, false),
        llvm::Function::ExternalLinkage,
        "__quantum__rt__qubit_allocate", module.get());
    ...
    runtime.qubitAllocateArray = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__qubit_allocate_array", module.get());
    ...
    runtime.qubitRelease = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__qubit_release", module.get());
    ...
    runtime.qubitReleaseArray = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__qubit_release_array", module.get());
}
...
void QIRCodeGen::declareResultManagement() {
    ...
    runtime.resultGetOne = llvm::Function::Create(
        llvm::FunctionType::get(resultPtrType, {}, false),
        llvm::Function::ExternalLinkage,
        "__quantum__rt__result_get_one", module.get());

    // __quantum__rt__result_get_zero
    runtime.resultGetZero = llvm::Function::Create(
        llvm::FunctionType::get(resultPtrType, {}, false),
        llvm::Function::ExternalLinkage,
        "__quantum__rt__result_get_zero", module.get());
    ...
    runtime.resultEqual = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__result_equal", module.get());
    ...
    runtime.recordBool = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__bool_record_output", module.get());
}
...
void QIRCodeGen::declareQuantumGates() {
    ...
    llvm::FunctionType* rotType = llvm::FunctionType::get(voidTy, {doubleTy, qubitPtrType}, false);
    runtime.qisRx = llvm::Function::Create(rotType, llvm::Function::ExternalLinkage, "__quantum__qis__rx__body", module.get());
    runtime.qisRy = llvm::Function::Create(rotType, llvm::Function::ExternalLinkage, "__quantum__qis__ry__body", module.get());
    runtime.qisRz = llvm::Function::Create(rotType, llvm::Function::ExternalLinkage, "__quantum__qis__rz__body", module.get());
    
    // 2-Qubit Gates
    llvm::FunctionType* twoQType = llvm::FunctionType::get(voidTy, {qubitPtrType, qubitPtrType}, false);
    runtime.qisCNOT = llvm::Function::Create(twoQType, llvm::Function::ExternalLinkage, "__quantum__qis__cnot__body", module.get());
    runtime.qisCZ   = llvm::Function::Create(twoQType, llvm::Function::ExternalLinkage, "__quantum__qis__cz__body", module.get());
    runtime.qisSWAP = llvm::Function::Create(twoQType, llvm::Function::ExternalLinkage, "__quantum__qis__swap__body", module.get());
    
    // 3-Qubit Gates
    llvm::FunctionType* threeQType = llvm::FunctionType::get(voidTy, {qubitPtrType, qubitPtrType, qubitPtrType}, false);
    runtime.qisCCNOT = llvm::Function::Create(threeQType, llvm::Function::ExternalLinkage, "__quantum__qis__ccnot__body", module.get());
    
    // Measurement: %Result* @__quantum__qis__m__body(%Qubit*)
    // Note: Correct instruction is 'm', not 'mz', and it returns Result*
    llvm::FunctionType* measType = llvm::FunctionType::get(resultPtrType, {qubitPtrType}, false);
    runtime.qisMeasure = llvm::Function::Create(measType, llvm::Function::ExternalLinkage, "__quantum__qis__m__body", module.get());
}
...
void QIRCodeGen::declareArrayHelpers() {
    ...
    runtime.arrayGetElement = llvm::Function::Create(
        ...
        llvm::Function::ExternalLinkage,
        "__quantum__rt__array_get_element_ptr_1d", module.get());
}
...
void QIRCodeGen::declareIOFunctions() {
    ...
    llvm::FunctionType* printfType = llvm::FunctionType::get(
        builder->getInt32Ty(), {i8PtrType}, true);
...
void QIRCodeGen::generateCode(const std::vector<ASTNode*>& nodes) {
    // 1. Create the Synthesized Main Function
    // This acts as the entry point for your script-like language
    llvm::FunctionType* mainType = llvm::FunctionType::get(builder->getVoidTy(), {}, false);
    llvm::Function* mainFunc = llvm::Function::Create(
        mainType, llvm::Function::ExternalLinkage, "main", module.get());
    
    // Required by QIR runner to find the start
    mainFunc->addFnAttr("EntryPoint"); 
    
    // 2. Create the Entry Block
    ...
    for (auto* node : nodes) {
        if (auto* funcDecl = dynamic_cast<FunctionDecl*>(node)) {
            ...
            // We must pause 'main' generation, generate the new function, then resume 'main'.
            
            // a. Save current spot in 'main'
    ...
    // 4. Finalize Main
    // Ensure the main function returns void at the end
...
void QIRCodeGen::visit(FunctionDecl* node) {
    ...
    // *** FIX: Add EntryPoint Attribute ***
    // Checks for "main" or "Main" and adds the required QIR attribute
...
void QIRCodeGen::visit(VariableDecl* node) {
    ...
    if (auto* base = dynamic_cast<BaseTypeNode*>(node->getVariableType())) {
        ...
        if (auto* baseElem = dynamic_cast<BaseTypeNode*>(arr->getElementType())) {
            if (baseElem->getBaseType() == BaseTypeKind::QUBIT) isQuantum = true;
        }
        // Determine static size if possible
        if (arr->getDimension()) {
    ...
    if (currentFunction) {
        ...
        if (isQuantum) {
            ...
            llvm::AllocaInst* ptrStorage = builder->CreateAlloca(llvmType, nullptr, varName);
...
void QIRCodeGen::visit(IndexAccessExpr* node) {
    node->getTarget()->visit(this); // Visits identifier, loads the Array* (from the alloca)
    llvm::Value* arrayPtr = lastValue;
    
    node->getIndex()->visit(this);
    llvm::Value* idx = lastValue;
    
    if (!arrayPtr || !idx) { lastValue = nullptr; return; }

    // Call runtime: returns i8* (pointer to the slot)
    llvm::Value* rawSlotPtr = builder->CreateCall(runtime.arrayGetElement, {arrayPtr, idx});
    ...
    llvm::Type* qubitSlotTy = llvm::PointerType::getUnqual(*context); // %Qubit** (opaque pointers in LLVM 15+ are just ptr)
    ...
    if (node->getTarget()) {
        ...
        if (targetId) {
            ...
            if (sym) {
                ...
                llvm::Value* isEqual = builder->CreateCall(runtime.resultEqual, {resultPtr, oneObj});
...
void QIRCodeGen::visit(ResetStmt* node) {
    ...
    // 2. Call Reset Runtime Function
    // (runtime.qisReset was initialized in declareQuantumGates)
    ...
    if (node->getArgument()) {
        ...
        if (val) {
            ...
            if (val->getType()->isIntegerTy(1)) {
                ...
                llvm::Value* fmt;
                if (val->getType()->isIntegerTy()) fmt = builder->CreateGlobalString("%lld\n");
                else if (val->getType()->isDoubleTy()) fmt = builder->CreateGlobalString("%f\n");
                else fmt = builder->CreateGlobalString("%s\n");
...
llvm::Value* QIRCodeGen::applyGate(GateKind gate, llvm::Value* target, const std::vector<llvm::Value*>& params) {
    llvm::Function* f = nullptr;
    switch(gate) {
...
llvm::Type* QIRCodeGen::getLLVMType(Type* type) {
    ...
    if (auto* b = dynamic_cast<BaseTypeNode*>(type)) {
        switch (b->getBaseType()) {
            ...
            case BaseTypeKind::QUBIT: return llvm::PointerType::getUnqual(*context);
...
void QIRCodeGen::visit(IntLiteralExpr* n) { lastValue = llvm::ConstantInt::get(builder->getInt64Ty(), n->getValue()); }
void QIRCodeGen::visit(FloatLiteralExpr* n) { lastValue = llvm::ConstantFP::get(builder->getDoubleTy(), n->getValue()); }
void QIRCodeGen::visit(BoolLiteralExpr* n) { lastValue = llvm::ConstantInt::get(builder->getInt1Ty(), n->getValue()); }
void QIRCodeGen::visit(StringLiteralExpr* n) { lastValue = builder->CreateGlobalString(n->getValue()); }
...
void QIRCodeGen::visit(FunctionCallExpr* node) {
    ...
    for (size_t i = 0; i < argExprs.size(); ++i) {
        ...
        // Basic Type Promotion (e.g., passing int to double arg)
        if (i < func->arg_size()) {
            ...
            if (lastValue->getType() != paramType) {
                ...
                // Add more implicit casts here if needed
            }
        }
        args.push_back(lastValue);
    }
...
void QIRCodeGen::visit(LambdaExpr* n) {
    std::cerr << "Warning: Lambda expressions are not yet supported in QIR backend.\n";
    lastValue = nullptr;
}
...
void QIRCodeGen::visit(DeclarationStmt* n) { for(auto d : n->getDeclarations()) d->visit(this); }
...
void QIRCodeGen::visit(IfStmt* node) {
    ...
    // 2. Create Blocks (Pass 'func' to attach them immediately)
    llvm::BasicBlock* thenBB = llvm::BasicBlock::Create(*context, "if.then", func);
    // Attach elseBB to func immediately if it exists
    llvm::BasicBlock* elseBB = node->getElseBlock() ? llvm::BasicBlock::Create(*context, "if.else", func) : nullptr;
    // Attach mergeBB to func immediately
    ...
    // Only branch to merge if the block wasn't already terminated (e.g. by return)
    if (!builder->GetInsertBlock()->getTerminator()) {
    ...
    // 5. Else Block
    if (elseBB) {
        // No need to push_back(elseBB) here; it's already attached.
    ...
    // 6. Merge Block
    // No need to push_back(mergeBB) here; it's already attached.
...
void QIRCodeGen::visit(ForStmt* node) {
    ...
    // 1. Init
    if (node->getInit()) {
        // Wrap init in a scope (blocks usually handle this via CompoundStmt, but strictly here:)
        enterScope(); 
...
void QIRCodeGen::visit(BreakStmt* node) {
    ...
    llvm::BasicBlock* nextBB = llvm::BasicBlock::Create(*context, "after.break", builder->GetInsertBlock()->getParent());
...
void QIRCodeGen::visit(ContinueStmt* node) {
    ...
    llvm::BasicBlock* nextBB = llvm::BasicBlock::Create(*context, "after.continue", builder->GetInsertBlock()->getParent());
    builder->SetInsertPoint(nextBB);
}
...
llvm::Value* QIRCodeGen::createBinaryOp(const std::string& op, llvm::Value* lhs, llvm::Value* rhs) {
    bool isFP = lhs->getType()->isDoubleTy() || rhs->getType()->isDoubleTy();
    
    // Math
    if (op == "+") return isFP ? builder->CreateFAdd(lhs, rhs) : builder->CreateAdd(lhs, rhs);
    if (op == "-") return isFP ? builder->CreateFSub(lhs, rhs) : builder->CreateSub(lhs, rhs);
    if (op == "*") return isFP ? builder->CreateFMul(lhs, rhs) : builder->CreateMul(lhs, rhs);
    if (op == "/") return isFP ? builder->CreateFDiv(lhs, rhs) : builder->CreateSDiv(lhs, rhs);
    if (op == "%") return builder->CreateSRem(lhs, rhs); // Integer only for now
...
llvm::Value* QIRCodeGen::createUnaryOp(const std::string& op, llvm::Value* operand) {
    if (op == "-") {
        return operand->getType()->isDoubleTy() ? builder->CreateFNeg(operand) : builder->CreateNeg(operand);
    }
    if (op == "!") {
...
llvm::Value* QIRCodeGen::createComparison(const std::string& op, llvm::Value* lhs, llvm::Value* rhs) {
