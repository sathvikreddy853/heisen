...
private:
    ...
    Expr* operand;
public:
    UnaryOpExpr(std::string op, Expr* operand)
...
private:
    ...
    Expr* index;
public:
    IndexAccessExpr(Expr* target, Expr* index)
...
private:
    ...
    Expr* index;
public:
    QuantumStateIndexAccess(IdentifierExpr* id, Expr* index)
...
public:
    QuantumStateList(std::vector<QuantumStateExpr*> states)
        : states(std::move(states)) {}
    ~QuantumStateList() {
        for (QuantumStateExpr* state : states) {
            delete state;
        }
    }
...
public:
    DeclarationStmt(std::vector<VariableDecl*> decls)
        : variableDeclarations(std::move(decls)) {}
...
public:
    PrintStmt(PrintOperationKind opKind, Expr* arg)
        : kind(opKind), argument(arg) {}
...
public:
    MatchCase(Expr* matchCase, Stmt* body)
        : matchCase(matchCase), body(body) {}
...
public:
    BaseTypeNode(BaseTypeKind t)
        : Type(Type::BaseType), baseType(t) {}
...
public:
    CompositeGateNode(std::vector<GateNode*> gates)
...
private:
    ...
    GateNode* rightGate;
public:
    GateCompositionNode(GateNode* left, GateNode* right)
