...
class ASTPrinter : public RecursiveASTVisitor {
    std::vector<bool> stateStack;
    bool useColor;

    void indent ();
    void printNode (const std::string& nodeType, const std::string& details = "");
    void printNode (const std::string& nodeType,
    const Heisen::Location& loc,
    const std::string& details = "");

    public:
    ASTPrinter (bool color = true) : useColor (color) {
    }

    void printTranslationUnit (const std::vector<ASTNode*>& nodes);
